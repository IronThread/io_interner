...
impl<'a, T> PartialEq for IOObj<'a, T> {
    fn eq(&self, other: &Self) -> bool {
        self.start_init == other.start_init
            && self.start == other.start
            && ptr::eq(self.guard, other.guard)
    }
}
...
pub struct IOInterner<T: Write + Read + Seek> {
    ...
    ///
    /// The [`IOObj`] comparison algorithm would mess up if you do not ensure that already
...
impl<T: Write + Read + Seek> IOInterner<T> {
    ...
    ///
    /// The `IOObj` returned does not implement `Write` as that would mess up equality of two
    /// `IOObj`ects pointing to the same position with same advanced cursor which it's actually
